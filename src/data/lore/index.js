...
export function getLoreForTrait(category, traitValue) {
  ...
  // E. Headwear Mapping Check
  if (mappedCategory === "Headwear") {
    const headwearAlias = HEADWEAR_MAP[valLower];
    if (headwearAlias) {
      const headwearKey = Object.keys(categoryDb).find(
        (key) => String(key).toLowerCase() === headwearAlias.toLowerCase()
      );
      if (headwearKey) return resolveLoreString(categoryDb[headwearKey]);
    }
  }

  // D. Attire Mapping Check
  if (mappedCategory === "Attire") {
    const attireAlias = ATTIRE_MAP[valLower];
    if (attireAlias) {
      const attireKey = Object.keys(categoryDb).find(
        (key) => String(key).toLowerCase() === attireAlias.toLowerCase()
      );
      if (attireKey) return resolveLoreString(categoryDb[attireKey]);
    }
  }
  ...
  if (mappedCategory === "Eyewear") {
    const normalizedName = normalizeEyewearName(valLower);
    if (normalizedName) {
      const eyeKey = Object.keys(categoryDb).find(
        (key) => String(key).toLowerCase() === normalizedName.toLowerCase()
      );
